...
if (data) {
  ...
  // будет ненужно, т.к. при парсинге данных сразу убираем ненужную строку
  let first = true;

  // выносим в отдельную функцию - работа с density
  let max = 0;

  // 1. получает новый массив table (разбиение по колонкам),
  // все данные в нем остаются в виде строки
  // будем столбец density преобразовывать в number при парсинге,
  // т.к. далее с ним надо делать математические операции
  // т.о. оптимизируем выделение памяти под массив
  // надо разделить парсинг данных и работу с подготовленными данными
  // (в парсинг включить первый split)
  for (const line of lines) {
    if (first) {
      ...
      // в отдельную функцию поиска maximum по столбцу density
      // (для этого его преобразуем в number)
      if (d > max) max = d;
  ...
  // а соединяем вычисленные данные со входными тоже в отдельной функции -
  ...
  // форматируем и выводим - две отдельные функции
  for (const row of table) {
